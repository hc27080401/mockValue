...
const formatRegexp = (() => {
    ...
    return RegExp(`\\\\[yMmdHhmsSAaTC]|(${matches.join('|')})`, 'g')
})()

module.exports = { date, datetime, time, day }
...
function date (date = new Date(), format = 'yyyy-MM-dd') {
    ...
    return format.replace(formatRegexp, (all, match) => match ? formatDefine[match](date) : all)
}
